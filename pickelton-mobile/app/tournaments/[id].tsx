...
import { apiErrorMessage, cancelTournamentRegistration, fetchTournament, fetchTournamentParticipants, getCurrentUser, uploadTournamentBanner, type Tournament, type TournamentParticipant } from "../../services/api";
...
export default function TournamentDetailsScreen() {
  ...
  async function register() { if (id.startsWith("preview-")) return setMessage("Preview event saved. Live registration opens when the event is published."); router.push({ pathname: "/tournaments/[id]/register", params: { id } }); }
  ...
  async function pickBanner() {
    ...
    if (!permission.granted) return setError("Photo permission is needed to upload a tournament banner.");
    ...
    try {
      setUploading(true);
      setItem(await uploadTournamentBanner(id, result.assets[0].uri));
      setMessage("Tournament banner uploaded.");
    } catch (cause) {
  ...
  const canManage = getCurrentUser()?.role === "ADMIN" || item.createdBy?.id === currentId;
  return <View style={styles.wrapper}><SafeAreaView><ScrollView contentContainerStyle={styles.content}><BackLink label="BACK TO EVENTS" /><BrandMark compact /><Text style={styles.title}>{item.name.toUpperCase()}</Text>
    {item.bannerUrl ? <Image source={{ uri: item.bannerUrl }} style={styles.banner} /> : <View style={styles.bannerFallback}><Text style={styles.bannerText}>TOURNAMENT</Text></View>}
    ...
    {canManage ? <PrimaryButton label={uploading ? "UPLOADING BANNER..." : "UPLOAD BANNER"} icon="image-outline" variant="outline" onPress={pickBanner} style={styles.secondary} /> : null}
    <PrimaryButton label="BUILD REGISTRATION FORM" variant="outline" onPress={() => router.push({ pathname: "/tournaments/[id]/form-builder", params: { id } })} style={styles.secondary} />
