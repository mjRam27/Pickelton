...
export function TextField({ label, error, helperText, id, className = "", ...props }: TextFieldProps) {
  ...
  return (
    <label className="block" htmlFor={inputId}>
      ...
      <input
        id={inputId}
        className={`min-h-12 w-full rounded-lg bg-black/72 px-4 py-3 text-sm font-semibold text-on-surface placeholder:text-on-surface-variant/70 outline outline-1 outline-outline/40 transition focus:field-focus focus:outline-primary/70 ${error ? "outline-error/80" : ""} ${className}`}
